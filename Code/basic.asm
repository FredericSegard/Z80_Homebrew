...
WRKSPC	.EQU	$1000			  ; BASIC Work space
...
TEMPSTACK	.EQU	$7FFF ; Top of BASIC line input buffer so is "free ram" when BASIC resets
...
BASIC:
	...
	ld		HL,SignOnMsg			; Sign-on message
	call	PrintString				; Output string
...
CheckIfCold:
	cp		"C"						; Is it C for Coldstart?
	jr		nz,CheckIfWarm			; No, then check if it's Warmstart
	call	PrintChar				; Print the C
	call	PrintCRLF				; Print carriage return and linefeed
...
SignOnMsg:
	db		"Updates to BASIC by Grant Searle",CR,LF
	db		"Adapted by Frederic Segard",CR,LF
...
SIGNON: 
	db	"Z80 BASIC Ver 4.7b",CR,LF
	db	"Copyright (C) 1978 by Microsoft",CR,LF,0
...
INITAB: JP		WARM			; Warm start jump
...
LISTT:	CALL	ATOH			; ASCII number to DE
...
FOR:	LD		A,64H			; Flag "FOR" assignment
		...
		PUSH	HL				; Save code string address
		CALL	DATAA			; Get next statement address
...
TSTBRK: 
	call	ReadCharNoWait			; Check for presence of character
	ret		z						; Return if nothing is in SIO buffer
	
;		call	CKINCHAR				; Check input status
...
STOPP:	RET		NZ				; Exit if anything else
		db	0F6H			; Flag "STOP"
...
NULLL:	CALL	GETINT			; Get integer 0-255
...
DATAA:	db	   01H,3AH		   ; ':' End of statement
...
MRPRNT: DEC		HL				; DEC 'cos GETCHR INCs
		CALL	GETCHR			; Get next character
PRINTT:	JP		Z,PRNTCRLF		; CRLF if just PRINT
...
FDTLP:	CALL	DATAA			; Get next statement
		OR		A				; End of line?
...
DEF:	CALL	CHEKFN			; Get "FN" and name
		...
		LD		BC,DATAA			; To get next statement
...
STRR:	CALL	TSTNUM			; Make sure it's a number
...
ASCC:	LD		BC,PASSA		; To return integer A
		PUSH	BC				; Save address
...
MULTT:	POP		BC				; Get number from stack
		POP		DE
...
POWER:	POP		BC				; Get base
		...
		JP		Z,EXPP			; Make result 1 if zero
...
EXPP:	CALL	STAKFP			; Put value on stack
...
GETINP: call	ReadChar				;input a character
		RET
...
;		JP		$0008			; output a char
		JP		PrintChar				; output a char


MONITR: 
;		JP		$0000			; Restart (Normally Monitor Start)
		JP		$8000			; Restart (Normally Monitor Start)
